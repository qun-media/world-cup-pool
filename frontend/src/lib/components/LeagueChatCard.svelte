<script lang="ts">
	...
	async function remove(message: ChatMessage) {
		await leagueChat.delete(message.id);
		deleteId = '';
	}
			...
			{#each leagueChat.messages as message (message.id)}
				...
				<article class="message" class:mine>
					...
					<div
						...
						onclick={(e) => onBubbleTap(e, message.id)}
					>
						<div class="meta">
							<strong>{mine ? copy.you : message.user.name}</strong>
							<span>{timeLabel(message.created)}{message.editedAt ? ` · ${copy.edited}` : ''}</span>
						</div>
						...
						{#if deleteId === message.id}
							<div class="delete-confirm">
								<span>{copy.deleteConfirm}</span>
								<button class="mini danger" onclick={() => remove(message)}>{copy.delete}</button>
								<button class="mini" onclick={() => (deleteId = '')}>{copy.cancel}</button>
							</div>
						{/if}
